...
import cn.yiyang.system.shiro.util.common.JsonConvertUtil;
...
public class JWTFilter extends BasicHttpAuthenticationFilter {
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        if (JedisUtil.exists(Constant.PREFIX_SHIRO_REFRESH_TOKEN + userId)) {
            ...
            String currentTimeMillisRedis = (String) JedisUtil.getObject(Constant.PREFIX_SHIRO_REFRESH_TOKEN + userId);
            ...
            if (JwtUtil.getClaim(token, Constant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                ...
                String[] userRoles = JwtUtil.getClaim(token, Constant.USERROLES, "roles");
                token = JwtUtil.sign(userId, userRoles, currentTimeMillis);
    ...
    private void response401(ServletResponse response, String msg) {
        ...
        try (PrintWriter out = httpServletResponse.getWriter()) {
            String data = JsonConvertUtil.objectToJson(new ResultBean<>(HttpStatus.UNAUTHORIZED.value(), "无权访问(Unauthorized):" + msg, null));
            out.append(data);
        } catch (IOException e) {
            logger.error("直接返回Response信息出现IOException异常:" + e.getMessage());
            throw new WYYException("直接返回Response信息出现IOException异常:" + e.getMessage());
        }
    }
}
